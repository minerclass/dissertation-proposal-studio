...
* ~~Sibling repositories still carry the dropped case-study framing: `dissertationquestionsbeta` (23 matches) and `dissertation-overview` (3 matches) as of this date.~~ **Correction, 2026-08-09.** Those counts were wrong. They were measured against local clones that were out of date: `dissertationquestionsbeta` was 12 commits behind `origin/main`, and `dissertation-overview` was sitting on an abandoned July 6 branch rather than `main`. Both repositories were in fact aligned on 2026-07-15 by merged pull requests titled "Align public proposal with constructivist methodology" (`dissertation-overview` #1, `dissertationquestionsbeta` #2). Verified against `origin/main` in both: `dissertation-overview` has no remaining occurrences, and the three in `dissertationquestionsbeta` are all intentional (see next bullet). No sibling-repository edits were needed or made.
* The three remaining case-study occurrences in `dissertationquestionsbeta` are deliberate and must not be edited. One, at `qualifying-paper-final-review/index.html:283`, is the reader's note stating that the July 17, 2026 proposal does not use a case-study label. The other two are inside the verbatim text of the completed qualifying paper, which said the dissertation would use a mixed-methods case study design. That manuscript is a finished, approved document and altering it would misrepresent the scholarly record; the note at line 283 exists precisely so the superseded wording can stand without misleading a reader. A fourth match, in `.github/workflows/ci.yml`, is the terminology linter's list of forbidden phrases.
